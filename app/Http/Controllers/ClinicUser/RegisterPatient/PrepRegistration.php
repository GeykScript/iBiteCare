...
class PrepRegistration extends Controller
{
    ...
    public function registerPatientPrep(RegisterPatientPREPRequest  $request)
    {
        ...
        try{
            ...
            Log::error('Error registering PREP patient: ' . $e->getMessage(), [
                'stack' => $e->getTraceAsString()
            ]);
            $id = $request->service_id;
            return redirect()->route('clinic.patients.register.prep', ['id' => $id])->with('error', 'An error occurred while processing your request.');
    }
}
}
