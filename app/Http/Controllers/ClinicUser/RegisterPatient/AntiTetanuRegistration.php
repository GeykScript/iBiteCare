...
class AntiTetanuRegistration extends Controller
{
    ...
    public function registerPatientAntiTetanu(RegisterPatientAntiTetanusRequest $request)
    {
        ...
        try{
            ...
            Log::error('Error registering Anti Tetanus patient: ' . $e->getMessage(), [
                'stack' => $e->getTraceAsString()
            ]);
            $id = $request->service_id;
            return redirect()->route('clinic.patients.register.anti-tetanus', ['id' => $id])->with('error', 'An error occurred while processing your request.');
    }
}
}
